...
import type { Referral, ReferralUpdate } from '@/types/database.types'
...
import { updateReferral } from '@/lib/supabase/queries'
import { useToast } from '@/hooks/use-toast'
import { logReferralUpdated } from '@/lib/supabase/actions/activity-actions'
...
export default function EditReferralDialog({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const updatedReferral = await updateReferral(referral.id, updates)

      // Log the activity
      await logReferralUpdated(
        referral.id,
        updates.person_name || referral.person_name,
        'Updated referral details'
      )

      toast({
        title: 'Referral updated',
        description: `Successfully updated referral for ${updates.person_name}`,
      })
      onReferralUpdated(updatedReferral)
      onOpenChange(false)
    } catch (error) {
      toast({
        title: 'Error',
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                defaultValue={referral.person_name}
                required
              />
            </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              defaultValue={referral.linkedin_url || ''}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select name="relationship" defaultValue={referral.relationship || undefined}>
                <SelectTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                name="date_asked"
                type="date"
                defaultValue={referral.date_asked || ''}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                name="follow_up_date"
                type="date"
                defaultValue={referral.follow_up_date || ''}
              />
            </div>
          </div>
